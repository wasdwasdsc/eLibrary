...
def reg():
    form = RegisterForm()
    if form.validate_on_submit():
        if form.e_mail.data not in str(models.User.query.all()):
...
def delete():
    ...
    if form.validate_on_submit():
        ...
        if form.by_author.data == form.by_book.data == True:
            if form.text.data in str(models.Author.query.all()):
                a = models.Author.query.filter_by(name=form.text.data).first()
                db.session.delete(a)
            if form.text.data in str(models.Book.query.all()):
